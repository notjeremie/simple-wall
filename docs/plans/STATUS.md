...
- **Overnight run passed (reported 2026-07-20).** The operator reports the build ran in production overnight with no issues. This is an operator report, not an instrumented soak — no logs were pulled and no metrics captured, so treat it as "no visible failures over ~1 night of real use" rather than a formal soak sign-off. The fold lines were also re-checked on the hardware that morning, producing the calib7 correction above.
