...
- **EXPERIMENT shipped (SwapSettleMs=500):** after the incoming layer reports a vout, hold the outgoing clip in front for 500ms before flipping, to let the occluded layer paint while hidden. Decisive: if the visible black shrinks, occluded layers DO paint and this is tunable; if the black is unchanged and only the switch feels slower, occluded layers paint only once fronted and libvlc cannot be made seamless here. Instrumented (`first vout at …`, `settle done`, back Time). Awaiting the hardware log + the operator's eyes. This build also still carries the DPI fix + instrumentation.
