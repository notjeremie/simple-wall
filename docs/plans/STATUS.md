...
- **Fold calibration finalized AND confirmed on the wall.** The corner-wall creases were measured on the physical LED, played back, photographed, and corrected against the ruler: LEFT fold = **190px**, RIGHT fold = **1445px** on the 1664-wide canvas (a first pass at 175/1475 was off by ~15px left and ~30px right; the wall photos caught it). Final authoring reference: `dist/hotfix/calib6-folds-1664x256.mp4` — three faces (LEFT RETURN 0–190 = 190px, FRONT FACE 190–1445 = 1255px, RIGHT RETURN 1445–1664 = 219px), red keep-clear bands over each fold, tile ruler. Content authors keep logos/faces out of the red fold zones. This is a content-authoring aid, not app code — it does not touch the soak. (Superseded: the earlier `calib5-folds-1664x256.mp4` at 175/1475.)
- **Deployed and partially verified LIVE on the wall (2026-07-19).** The `9f46e59` build was deployed to the real Win7 wall. Two things confirmed on the hardware — the highest-risk paths: (1) the migration left the wall looking **identical** to before (each clip inherited the old global), and (2) **per-clip fader persistence works with real clip switching** — set a look, switch away, switch back, the look returns. Still to confirm next session (the wall went back into production use until 2026-07-20): replace-resets-the-look, sliders-grey-when-idle (both unit-tested + RenderShot-verified already), and that the calib5 fold lines sit on the physical creases.
