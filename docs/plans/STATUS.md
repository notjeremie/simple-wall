...
- **CONFIRMED FIXED on the Win10 hardware (2026-07-21).** The operator reports the output now lands at the correct size on reopen. The fix (re-assert bounds in `OnHandleCreated`, swallow `WM_DPICHANGED`) holds. The DPI instrumentation logging is still in the build and could be trimmed in a later cleanup, but it is harmless. Also confirmed earlier the same day: Task Manager shows the process as **Per-monitor (V2)**, and normal operation (play/geometry/sliders/stop) is unaffected.
