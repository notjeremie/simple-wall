...
- **The first manifest SHIPPED BROKEN and blocked launch on Win10 (fixed same day).** `--` is illegal inside an XML comment, and the comments used it as a dash. Windows rejects a malformed manifest during process creation with *"the application has failed to start because its side-by-side configuration is incorrect"* — before any of our code runs, so there is no log line and the message names no cause. Two things made it survive review: the build reports no warnings for a bad manifest, and **the ARM64 build VM tolerated it and launched anyway**, so a smoke test on the VM was not evidence. It was only reproducible on real Win10. Fixed by rewriting the comments and adding `type="win32"` to `assemblyIdentity` (also required, also missing).
- **A SECOND startup bug shipped right behind the first: `App.config` declared a built-in section.** On .NET Framework 4.7+ `System.Windows.Forms.ApplicationConfigurationSection` is built in, and declaring it in `<configSections>` is a hard error, not a harmless redundancy. It takes down the whole configuration system, so the first `ConfigurationManager` read throws and the app dies reporting *"Configuration system failed to initialize"*. Fixed by deleting the declaration and keeping only the section itself.
- **The VM smoke test proved unreliable and was replaced by a real parse test.** Three separate traps: the headless VM has no interactive session, the ARM64 config/manifest parsers are more lenient than a real desktop, and — worst — SimpleWall is **single-instance**, so a stuck `SimpleWall.exe` from an earlier run silently suppressed every later launch and produced no log at all, which reads exactly like success. `AppConfigTests.Real_configuration_system_can_parse_it` now hands the file to `ConfigurationManager.OpenMappedExeConfiguration` and forces every section to parse — the same code path that runs at startup, with no hardware and no launch involved.
- **`ManifestTests` (5) + `AppConfigTests` (4) now guard both files.** Well-formedness under both `XDocument` and a strict `XmlReader`, `type="win32"`, the Win10 `supportedOS` GUID, and the presence of `PerMonitorV2` + the `dpiAware` fallback. Verified the guard actually catches the real defect: against the broken manifest 4 of the 5 fail. This is the only file in the project that can kill the app completely while every other test passes and the build is clean.
- **Verified:** builds clean (0 warnings), 234/234 tests pass, the manifest is confirmed *embedded and valid XML* inside the packaged exe in `SimpleWall.exe` (the Win10 GUID and `PerMonitorV2` are present in the binary), and `SimpleWall.exe.config` carries the WinForms opt-in. **Not verified: actual behaviour on a scaled display.** That needs a Windows machine set to 125%/150% and cannot be proven from the Mac or from a headless VM — see Open threads.

### Clip-switch black frame: not seamless, never was (investigating)

- **Reported 2026-07-21 on both machines:** ~0.5s of black on every clip change. Confirmed from the logs it is NOT new — every log since v1 shows swaps of 180-860ms, and the black is that duration. The Win7 wall on the old v1.0 build shows it too, so it is not a regression.
- **The two-layer swap does not deliver a seamless cut on this hardware.** Every swap in the user's log takes the fast path (VoutCount>0, no "swapping anyway"), yet the wall is still black for the load. The front layer holds the outgoing clip during the load, so the black is almost certainly AFTER the flip: the newly-fronted incoming layer spends ~300-460ms painting its first frame because it did not paint while occluded. `VoutCount>0` means "vout created", not "frame presented" — a lying proxy on these GPUs.
- **I had overclaimed "no black frame / invisible" in the README and RUNBOOK; corrected 2026-07-21** to state the ~300-460ms reality and that frame-perfect switching is what a real playout engine (CasparCG — the studio-wall project) is for.
- **EXPERIMENT RUN AND CONCLUDED (2026-07-21): the occluded layer never paints. Reverted.** The settle build (hold the outgoing clip 500ms after the incoming reported a vout) produced this log on the real machine: `first vout at 338ms (back Time=0ms)` … `reported no picture within 1000ms -- swapping anyway` … `swapped after 1047ms`. Read it: the incoming layer created a vout at 338ms but its **playback clock was still 0** (no frame decoded), and during the settle the **vout was torn down** (VoutCount 1→0, which is the only way the later `SwapAnyway` timeout path fires) and the clock never advanced. So keeping the layer hidden did not let it pre-paint — libvlc **gave up on the hidden layer entirely**. On this hardware libvlc will not decode or paint an occluded window; the first frame is only ever painted once the layer is brought to the front, which is the ~300-460ms of black. The two-layer swap **cannot be made seamless here**, full stop. Experiment reverted (switch is back to the faster immediate-flip); finding is the deliverable.
- **Consequence:** for the corner wall (slow background loops) the ~300ms black is acceptable and always was. For the studio desk-graphics use (sharp cuts) it is not, and this is now hard evidence that frame-perfect switching needs the CasparCG-based studio product, not libvlc-in-a-window. This build carries the DPI fix + instrumentation; the black-frame experiment is gone.

### DPI bug: output renders too large on reopen (candidate fix, awaiting hardware log)

- **Symptom:** on a Win10 machine whose control monitor is not at 100% scaling, the wall output renders visibly too large (operator eyeballed ~150%). Nudging the width in Settings by +10/-10 fixes it until the next launch. Independent of the exact scale value.
- **Root cause (high confidence, from the code path):** `OutputWindow.Bounds` is assigned in the `VlcWallEngine` constructor (`ApplyGeometry` at ctor line 104) **before the window handle exists**. The handle is only created later, at the first `_outputWindow.Show()` inside `StartLoad`, in the DPI context of the control monitor where `MainForm` lives. WinForms then scales the window by the monitor's DPI ratio. Re-applying the identical geometry once the handle exists and the window is on the wall lands it correctly — which is exactly what the operator's Settings nudge does. `AutoScaleMode.None` does NOT prevent this; it governs control autoscaling, not the per-monitor `WM_DPICHANGED` window resize (an earlier comment wrongly claimed it did).
- **Fix, entirely inside `OutputWindow`:** remember the requested bounds; in `OnHandleCreated`, re-assert them once the handle is live (the automatic form of the manual nudge); and override `WndProc` to swallow `WM_DPICHANGED` so Windows can never rescale this fixed-pixel panel window. All three paths log `DeviceDpi`/`Bounds`/`ClientSize`.
- **Status: NOT yet confirmed on hardware.** This DPI behaviour cannot be reproduced on the headless single-DPI build VM, so like the DXVA2 crash it is a real-hardware-only bug. The build is deliberately instrumented: the log from a reproduce (close/reopen on the affected machine) will show the DeviceDpi and whether bounds were scaled and re-asserted, confirming the mechanism regardless of whether the fix holds. Builds clean, 238/238 tests pass. **Do not treat as fixed until the wall log confirms it.**
